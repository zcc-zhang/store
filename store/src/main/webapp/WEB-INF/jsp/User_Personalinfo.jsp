...
<style>
.user_center .right_style .title_Section span {
	border-bottom: 3px solid #F30;
	padding: 7px 10px;
}

.user_center .right_style .title_Section {
...
.Personal_info ul.xinxi .bottom input.modify {
	background: #F30;
	color: #fff;
}

.Personal_info ul.xinxi .bottom input.confirm {
	background: #F60;
	color: #FFF;
}
...
<body>
								...
								<li><label>用户性别：</label> <span class="sex"> <input
									...
									<div class="add_sex">
										<li><label>电子邮箱：</label> <span><input name="email"
												type="text" value="${user.email}" class="text" /></span></li>
	...
	<script type="text/javascript">
		...
		/***模拟鼠标点击***/
		document.querySelector('#shangchuan').addEventListener('click',
			function() {
				document.querySelector('#fileField').click();
			});
		$(function() {
			//绑定change事件
			$('#fileField').on('change', readerFile);
			/**********读取文件************/
			function readerFile() {
				//获取选择文件的信息
				var file = this.files[0];
			...
			function checkGender() {
				...
				if (gender == '' || gender == undefined) {
					$.sendWarningToTop('性别为必填项', 3000, function() {
			...
			$("input[name=submit]").on('click', function() {
				var headimg = $("input[type=file]").val(); //文件域
				var imgName = ''; //头像名字
				if (headimg != '') //如果文件域不为空则提交表单
				{
					var index = headimg.lastIndexOf("\\"); //找到字符串中/的位置
					var name = headimg.substring(index); //分割字符串
					$("input[name=headImgPath]").val(name); //赋值给隐藏域
					$.ajaxFileUpload( //提交到后台下载头像
						{
							url : "${pageContext.request.contextPath}/headImgUpLoadServlet",
				...
				if (!checkGender())
					flag = false;
				if (flag) {
					var dataStr = $("form").serialize(); //封装表单数据
					$.ajax({
						url : "${pageContext.request.contextPath}/userInfoAlterServlet",
						data : dataStr,
						type : "post",
						success : function(data) {
							...
							$('input[name=moblie]').val(dataJson.moblie);
							$('input[name=IDnumber]').val(dataJson.iDnumber);
							$('#headImg').attr('src', dataJson.headPortrait);
							$(".user_portrait img").attr('src', dataJson.headPortrait);
							$('input[type=file]').on('change', readerFile);
							console.log("头像:", $('#headImg').attr('src'))
							swal({
								title : "修改成功！",
								text : "您已经成功修改信息！。",
								type : "success"
